...
name of the run. A block asking for several radii at once (\var{radii}, or
\var{nradii} together with \var{r\_min} and \var{r\_max}, in place of \var{radius})
bundles them into one dump named
\var{<job>.r=<rmin>-<rmax>.<variable>.<counter>.vtk} instead, holding every surface
on the same angular grid. The pipeline reads both forms: it scans the sph directories
for the dumps of each variable and takes the surface at \var{-{}-radius} out of
whichever dump holds it. The blocks therefore need not agree on how they group their
radii---only on covering the requested one---and a run that switched form across a
restart is still analysed as one run. Useful radii are $300.0$ and $400.0$
(code units, $M_\odot$).
